...
bot.action("comandos_usuarios", (ctx) => {
  ctx.editMessageText(
    "Estos son los comandos disponibles a todos los Usuarios:",
    comandosUsuariosOptions
  );
});
...
bot.action("usuarios", async (ctx) => {
  ...
  const submenuMessage = await ctx.reply(
...
bot.action("menu_anterior", async (ctx) => {
  ...
  const comandosMessage = await ctx.reply(
    "Selecciona una opción:",
    comandosOptions
  );
...
bot.action("comandos_administradores", (ctx) => {
  ctx.editMessageText(
    "Estos son los comandos Extra para Administradores:",
...
bot.action("web", (ctx) => {
  ...
  ctx.replyWithHTML(
    `Para ver nuestra Web, da clic <a href="${urlWeb}">aquí</a>.`,
    { disable_web_page_preview: true }
  );
});
...
bot.action("denunciar", (ctx) => {
  // Redireccionar a la URL de WEB
  ctx.replyWithHTML(
    `Para hacer una Denuncia puedes, da clic <a href="${gDenun}">aquí</a>.`,
    { disable_web_page_preview: true }
  );
});
...
bot.action("reclamaciones", (ctx) => {
  // Redireccionar a la URL de WEB
  ctx.replyWithHTML(
    `Si fuiste sanciondo y quieres hacer una Reclamación, da clic <a href="${gRecla}">aquí</a>.`,
    { disable_web_page_preview: true }
  );
});
...
bot.action("soporte", (ctx) => {
  // Redireccionar a la URL de WEB
  ctx.replyWithHTML(
    `Si deseas entrar en el Grupos de Soporte Técnico, da clic <a href="${gSuppt}">aquí</a>.`,
    { disable_web_page_preview: true }
  );
});
...
bot.action("canal", (ctx) => {
  ...
  ctx.replyWithHTML(
    `Si quieres ver el Canal donde hemos publicado todos los casos, da clic <a href="${canalD}">aquí</a>.`,
    { disable_web_page_preview: true }
  );
});
...
bot.action("grupos", (ctx) => {
  ...
  ctx.replyWithHTML(
    `Para ver los grupos que recomendamos por su transparencia y seguridad, da clic <a href="${gRecom}">aquí</a>.`,
    { disable_web_page_preview: true }
  );
});
...
bot.action("nosotros", (ctx) => {
  ...
  ctx.replyWithHTML(
    `Para leer Sobre Nosotros, da clic <a href="${nosotr}">aquí</a>.`,
    { disable_web_page_preview: true }
  );
});
...
bot.action("reglas", (ctx) => {
  ...
  ctx.replyWithHTML(
    `Para ver nuestras Reglas, da clic <a href="${rules}">aquí</a>.`,
    { disable_web_page_preview: true }
  );
});
...
bot.action("salir", (ctx) => {
  ...
  ctx.reply(
    "No dudes en regresar a nuestro menú principal cuando quieras. Estamos aquí para ayudarte en lo que necesites. ¡Gracias por usar FirewallIds! 😊"
  );
